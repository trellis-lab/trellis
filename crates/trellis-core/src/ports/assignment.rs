...
pub fn assign_ports(graph: &Graph, cell_size: f64) -> HashMap<usize, EdgePorts> {
    ...
    for node in &graph.nodes {
        ...
        for (&side, edge_list) in &sides {
            ...
            for (i, info) in edge_list.iter().enumerate() {
                ...
                let grid_col = (port_x / cell_size).round() as i64;
                let grid_row = (port_y / cell_size).round() as i64;
...
mod tests {
    ...
    fn test_assign_ports_simple_vertical() {
        ...
        let ports = assign_ports(&graph, 10.0);
